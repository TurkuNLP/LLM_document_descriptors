...
def log_execution_time(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logging.info(
            f"Execution of {func.__name__} took {time.strftime('%H:%M:%S', time.gmtime(execution_time))}."
        )
        return result
...
def load_documents(source, cache):
    ...
    if source.lower() in hf_shorthands:
        ...
        try:
            return load_dataset(source, split="train", streaming=True, cache_dir=cache)
        ...
        with open(path, "r") as f:
            ...
            for line in file:
                try:
                    ...
                    logging.warning(
                        f"Skipping malformed line in descriptor file: {line}"
                    )
                    continue
        return descriptors
