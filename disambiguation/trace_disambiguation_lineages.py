...
def count_root_occurrences(root_ids: Set[str], original_ids) -> Dict[str, int]:
    ...
    for root_id in root_ids:
        if root_id not in id_counts:
            id_counts[root_id] = 1
            print(
                f"Warning: root ID {root_id} not found in original data, defaulting to count=1"
            )
...
def analyze_lineage(
    ...
    if duplicates:
        ...
        final_id_info = {
            r["final_id"]: (r["descriptor"], r["explainer"]) for r in results
        }
    ...
    if not untraceable and not duplicated:
        print(
            "Sanity check passed: All original inputs trace to exactly one final pair."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analyze lineage of descriptor-explainer pairs"
    )
    parser.add_argument(
        "--final",
        type=Path,
    ...
    parser.add_argument(
        "--output", type=Path, required=True, help="Output path for analysis"
    )

    args = parser.parse_args()
    checks_passed = analyze_lineage(
        args.final, args.lineage, args.original, args.output
    )
